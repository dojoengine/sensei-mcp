...
export const Logger = {
  ...
  spans: new Map<string, { startTime: number, metadata: Record<string, unknown> }>(),
  ...
  formatMessage(level: string, message: string, metadata?: Record<string, unknown>): string {
    const timestamp = new Date().toISOString();
    const levelColor = this.LEVEL_COLORS[level as keyof typeof Logger.LEVEL_COLORS];
    
    // Colorize the timestamp with dim color
    const coloredTimestamp = this.colorize(`[${timestamp}]`, "DIM");
    
    // Colorize the level
    const coloredLevel = this.colorize(level.padEnd(5), levelColor.label as keyof typeof Logger.COLORS);
    
    // Colorize the message
    const coloredMessage = this.colorize(message, levelColor.text as keyof typeof Logger.COLORS);
  ...
  span(name: string, metadata?: Record<string, unknown>): { end: (outcome?: string) => void } {
